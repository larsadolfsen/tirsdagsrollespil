      ...
      {options.map((option) => {
        ...
        return (
          <label key={option.id} className="group cursor-pointer">
            ...
            <span
              className={cn(
                "wfrp-label inline-flex h-6 items-center justify-center rounded border px-4 transition-colors",
                "peer-focus-visible:outline-none peer-focus-visible:ring-1 peer-focus-visible:ring-wfrp-gold/60",
                isSelected
                  ? "border-transparent bg-wfrp-gold text-primary-foreground"
                  : "border-transparent bg-white/10 text-wfrp-muted-text group-hover:bg-white/15 group-hover:text-gray-100",
              )}
            >
              {option.label}
            </span>
