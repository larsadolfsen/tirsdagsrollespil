...
test("advance XP controls use table layout and hide unavailable removals", async ({ page }) => {
  ...
  const advanceTab = page.getByRole("tabpanel", { name: /advance/i });

  await expect(advanceTab.getByText("XP", { exact: true }).first()).toBeVisible();
  await expect(advanceTab.getByText("Current", { exact: true }).first()).toBeVisible();
  await expect(advanceTab.getByText("Total", { exact: true }).first()).toBeVisible();
  await expect(advanceTab.getByText("Adjust", { exact: true }).first()).toBeVisible();
  await expect(advanceTab.getByText("Experience", { exact: true }).first()).toBeVisible();
